...
from dataset.dataset_helper import get_default_dataset_name, get_real_test_dataset_name, get_path_to_real_test_dataset, \
    check_path_exist
...
def get_real_test_dataset(number_of_batches, file_names, i, user_args):
    ...
    real_test_dataset_name = get_real_test_dataset_name(sliceSize) + "_{}_{}".format(batch_number,  # TODOx look inside
                                                                                     number_of_batches)
    my_logger.info("[+] Dataset name: " + real_test_dataset_name)
    ...
    if not os.path.isfile(path_to_dataset):
        ...
        return create_real_test_dataset_from_slices(sliceSize, file_names[starting_file:ending_file],
                                                    real_test_dataset_name, user_args)  # TODOx look inside
    else:
        my_logger.debug("[+] Using existing dataset")
